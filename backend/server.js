...
const BASE_INSTRUCTIONS = `CRITICAL - Food logging: When the user asks to "log" a meal, "help me log", or shares a meal photo/description, you MUST end your reply with a [FOOD_LOG] JSON block. The app uses this to show checkmarks so they can add items with one tap. You are NOT "logging" for them—you are providing the list; the app does the rest. FORBIDDEN: Do not say "I can't log", "I can't directly log", "I can't log the meal for you", "write it down", "enter it into your app", or "just add this to your log". Those are wrong. The correct response is: one short sentence like "Tap the checkmark next to each item to add it to your Food log." then the [FOOD_LOG] block with every ingredient (name, calories, protein, quantity). No text after the block.
